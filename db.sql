...
CREATE TABLE collages (id integer primary key, name text, sessionId text, data text, preview text, filename text, updated timestamp default (datetime('now','localtime')));
CREATE TRIGGER collages_updated after update on collages for each row begin update collages set updated = datetime('now','localtime') where id = OLD.id; end;
